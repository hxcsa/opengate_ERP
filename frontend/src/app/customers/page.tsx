...
import { Users, Plus, Search, X, Edit3, ChevronLeft, ChevronRight } from "lucide-react";
                ...
                {loading ? <div className="p-8 text-center text-slate-400 font-bold">Loading...</div> : error ? <div className="p-8 text-center text-rose-500 font-bold">{error}</div> : customers.length === 0 ? <div className="p-12 text-center"><Users size={48} className="mx-auto text-slate-200 mb-4" /><h3 className="text-lg font-black text-slate-400">No customers found</h3></div> : (
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100"><tr>
                            ...
                            <th className="px-5 py-4 text-[10px] font-black text-slate-400 uppercase tracking-wider">City</th>
                        ...
                        <tbody className="divide-y divide-slate-50">{customers.map(c => (
                            <tr key={c.id} className="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td className="px-5 py-4 text-sm text-slate-500">{c.address?.city || "—"}</td>
                                ...
                                <td className="px-5 py-4 text-right"><div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">{canModify && <button onClick={() => openEdit(c)} className="p-2 text-blue-400 hover:text-blue-700 hover:bg-blue-50 rounded-xl" title="Edit"><Edit3 size={16} /></button>}</div></td>
                            </tr>
                        ))}</tbody>
                    </table>
                )}
